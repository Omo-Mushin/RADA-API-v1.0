...
# from API_STORE import OPENAI_API_KEY, PINECONE_API_KEY
# -------------------------------
# Configuration
# -------------------------------
load_dotenv()
...
# OpenAI Model Options - Change this to test different models
...
def query_chatbot_with_compute(collection, embedding_model, reranker, openai_client,
    ...
    for query in queries[:3]:
        ...
        result = collection.query(
            ...
            include_values=False  # optional, saves bandwidth
        )

        # Pinecone v2 returns 'matches' with 'metadata'
        for match in result['matches']:
            text = match.metadata.get('text', '')  # make sure your upsert included 'text' field
            if text:
...
**ANSWER (be direct and well-formatted):**"""
    ...
    return openai_llm_inference(openai_client, prompt)

# -------------------------------
# OpenAI LLM Inference
# -------------------------------
def openai_llm_inference(openai_client, prompt: str) -> str:
    """Call OpenAI API safely with version compatibility"""
    try:
        ...
        if hasattr(choice, 'message') and 'content' in choice.message:
            ...
            return "⚠️ OpenAI API returned unexpected format."
...
def chatbot(debug=False):
    ...
    print("\n" + "="*80)
    print(f"🧠 RADA Petroleum Engineering Assistant (OpenAI {OPENAI_MODEL})")
    print("="*80)
    ...
    while True:
        ...
        try:
            ...
            answer = query_chatbot_with_compute(
                ...
                openai_client, question, debug=debug
            )
            print("\n" + "="*80)
            print("📊 ANSWER:")
            print("="*80)
